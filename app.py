...
from sklearn.linear_model import LinearRegression # Đã thêm thư viện Machine Learning để tính Residual
...
def load_and_prep_data():
    ...
    try:
        ...
        # DOWNCASTING TO CATEGORY ĐỂ TĂNG TỐC RAM
...
# ==============================================================================
# APPLY LIMITS & CALC PASS/FAIL (TỐI ƯU HÓA VECTORIZATION)
# ==============================================================================
...
if view_mode == "✨ Gloss Trend (SPC)":
    ...
    if list_ma_son_tab2:
        ...
        # =========================================================================
        # TAB 4: DFT IMPACT + RESIDUAL ANALYSIS (NEW)
        # =========================================================================
        with tab_dft:
            st.subheader("📏 Process vs. Material (Thickness Correlation & Residuals)")
            ...
            if missing_cols:
                ...
                if not valid_targets:
                    ...
                    if len(df_plot) > 5:
                        ...
                        # --- BỔ SUNG: PHÂN TÍCH PHẦN DƯ (RESIDUAL ANALYSIS) ---
                        st.markdown("#### 3. Phân tích Phần dư (Residual Analysis - Root Cause)")
                        st.caption("Tách biệt lỗi nội tại của nhà cung cấp sơn (Material) ra khỏi sai lệch do độ dày màng sơn từ người vận hành (Process).")
                        ...
                        col_r1.metric("Biến động do Quy trình (R²)", f"{r2_val:.2f}", "Phụ thuộc vào độ dày")
                        col_r2.metric("Biến động do Sơn (Residual Std)", f"{res_std:.2f} GU", "Thiếu ổn định nội tại", delta_color="inverse" if res_std > 1.5 else "normal")
                        ...
                        ax_res.set_title("Biểu đồ Phần dư (Biến động độ bóng đã loại bỏ hoàn toàn yếu tố độ dày)", fontweight='bold')
                        ax_res.legend()
                        st.pyplot(fig_res)
                        plt.close(fig_res)
...
# ==========================================
# VIEW 6: MASTER SUMMARY REPORT (+ PARETO)
# ==========================================
    ...
    # --- BỔ SUNG: BIỂU ĐỒ PARETO (TOP LỖI) ---
    st.markdown("---")
