...
# ==========================================
# ==========================================
# VIEW 5: SUPPLIER BENCHMARKING (APPLES-TO-APPLES)
# ==========================================
elif view_mode == "🤝 Supplier Capability":
    st.header("🤝 Supplier Capability Benchmarking")
    st.info("💡 Logic: So sánh trực diện các nhà cung cấp dựa trên cùng một tiêu chuẩn (Cùng mã màu 4 số & Cùng mức Gloss mục tiêu).")
    
    # Tiền xử lý dữ liệu: Tính Target để phân loại
    dff_v5 = dff.dropna(subset=['Online_Gloss_Top', 'Supplier', 'Gloss_LSL', 'Gloss_USL', 'Color_Code']).copy()
    ...
    with col_f1:
        # Bước 1: Chọn mã màu 4 số (ví dụ: 1014, 9002...)
        list_color_code = sorted(dff_v5['Color_Code'].unique().tolist())
        sel_color_code = st.selectbox("🔢 Bước 1: Chọn Mã màu (4 số cuối):", list_color_code)
        
    with col_f2:
        # Bước 2: Chỉ hiện các mức Gloss mục tiêu có trong mã màu đó
        dff_filtered_code = dff_v5[dff_v5['Color_Code'] == sel_color_code]
        list_targets = sorted(dff_filtered_code['Gloss_Target'].unique().tolist())
        sel_target = st.selectbox("🎯 Bước 2: Chọn Mức Gloss mục tiêu (Target):", 
                                  list_targets, 
                                  format_func=lambda x: f"Tiêu chuẩn {x:.1f} GU")

    # Lọc dữ liệu cuối cùng theo đúng Mã màu & Đúng Target
    dff_final = dff_filtered_code[dff_filtered_code['Gloss_Target'] == sel_target].copy()
    
    # Chỉ giữ lại Supplier có ít nhất 2 mẻ (Batch) để đảm bảo tính thống kê
    ...
    if not dff_final.empty:
        st.markdown("---")
        st.subheader(f"📊 So sánh năng lực cung cấp: Mã màu {sel_color_code} | Target {sel_target} GU")
        
        c1, c2 = st.columns([2, 2.5])
        
        with c1:
            # Biểu đồ Boxplot so sánh trực diện các Supplier
            fig_bench, ax_bench = plt.subplots(figsize=(10, 6))
            sns.boxplot(data=dff_final, x='Supplier', y='Online_Gloss_Top', palette='vlag', ax=ax_bench, showfliers=False)
            sns.stripplot(data=dff_final, x='Supplier', y='Online_Gloss_Top', color='black', alpha=0.3, jitter=True, ax=ax_bench)
            
            # Vẽ đường Spec chung
            lsl_val = dff_final['Gloss_LSL'].iloc[0]
            usl_val = dff_final['Gloss_USL'].iloc[0]
            ax_bench.axhline(sel_target, color='green', ls='-', lw=2, label=f'Target ({sel_target})')
            ax_bench.axhline(lsl_val, color='red', ls='--', lw=1.5, label=f'LSL ({lsl_val})')
            ax_bench.axhline(usl_val, color='red', ls='--', lw=1.5, label=f'USL ({usl_val})')
            
            ax_bench.set_ylabel("Độ bóng thực tế trên Line (GU)")
            ax_bench.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
            st.pyplot(fig_bench)
            
        with c2:
            # Bảng so sánh các chỉ số "vàng"
            ...
            # Tính Bias (Độ lệch so với Target)
        ...
        st.markdown("---")
        st.subheader("🕵️ Chi tiết biến động theo từng Mẻ (Batch-to-Batch)")
        st.caption("Xem xét sự ổn định của từng mẻ sơn cụ thể giữa các nhà cung cấp.")
        
        # Biểu đồ Line chart so sánh xu hướng các mẻ
        fig_trend, ax_trend = plt.subplots(figsize=(14, 5))
        for sup in dff_final['Supplier'].unique():
            sup_data = dff_final[dff_final['Supplier'] == sup].groupby('Batch_Lot')['Online_Gloss_Top'].mean().reset_index()
            ax_trend.plot(sup_data['Batch_Lot'].astype(str), sup_data['Online_Gloss_Top'], marker='o', label=sup)
            
        ax_trend.axhline(sel_target, color='green', ls='-', alpha=0.3)
        ax_trend.set_ylabel("Gloss Mean per Batch")
        ax_trend.set_xlabel("Batch Lot Number")
        plt.xticks(rotation=45)
        ax_trend.legend()
        st.pyplot(fig_trend)
        
    else:
        st.warning(f"⚠️ Không có đủ dữ liệu so sánh cho mã màu {sel_color_code} ở mức Target {sel_target} GU (Yêu cầu ít nhất 2 mẻ/nhà cung cấp).")
